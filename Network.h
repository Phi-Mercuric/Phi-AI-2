...
namespace phi
{
	...
	public:
		...
		void compress() // goes through nodes and weights and compresses the values down.
		{
		...
		void backprop(vector<float> outputValues, const bool debug = false)
		{
			if (debug) {
			...
			for (int layer = layers - 1; layer >= 0; layer--)
			{
				...
				if (layer == layers - 1)
				{
					for (int node = 0; node < nodes[layer]; node++)
					{
						...
						net[layer][node].add_ordered_cords(xCords, yCords, debug);																								// in bellcurve of prev function
					}
				}
				else if (layer == 0)
				{
					for (int node = 0; node < nodes[layer]; node++)
					{
						...
						net[layer][node].add_ordered_cords(xCords, yCords, debug);
						if (debug) {
							cout << "\n      BPNode: " << node;
		...
		void calculate(vector<float> inputValues, bool debug = false)
		{
			if (debug) {
